...
const PublishModal = ({ publishBlog, setShowModal, notify }) => {
  const handleClose = () => {
    setShowModal(false);
  };
  ...
  let currentlyUploadingBlog = false;
        ...
        <div className="w-full xxs:w-1/2">
          <div className="flex flex-col mt-4">
            <label className="text-gray-200 text-sm ml-1.5 mb-1 flex gap-0.5 relative">
              <span className="text-red-500 absolute -left-1.5 -top-0.5">
                *
              </span>
              Blog Title
            </label>
      ...
      <button
        onClick={() => {
          if (
            ...
            if (currentlyUploadingBlog) {
              notify("A blog post is currently being uploaded, please wait");
            } else {
              //close modal
              handleClose();

              currentlyUploadingBlog = true;

              notify("Blog is being uploaded, please wait");
