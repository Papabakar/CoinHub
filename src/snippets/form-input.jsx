...
const FormInput = ({inputOptions, label}) => {
    ...
    return(
        <div className="relative">
            ...
            <label className={`${inputOptions.value.length ? `top-0 text-xs px-2 left-0 bg-main-bg after:content-['*'] after:text-red-500 after:ml-1  transition-all ease-in-out text-gray-100` : `top-1/2 left-3 text-xs` } -translate-y-1/2  absolute text-gray-500 `}>{`${label}`}</label>
        </div> 
    )
}
